...
description: Detailed protocol for the panma-harness supervisor (Main). Drives the multi-agent cycle (Designer → Executors → Verifier → Rule-Applier), Ralph loop, retry budget, worker check-in via background tasks, and persistent state via .harness/state.json. Loaded after harness mode activates.
---
...
- **Auto** (via project `CLAUDE.md` include): trigger conditions met → main calls `Skill(skill="loop", args="/harness-iterate <user request>")`.
...
- The `Relay` agent is not used — main absorbs user input directly.
